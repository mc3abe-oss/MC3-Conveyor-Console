...
export default function ConsoleSalesOrderDetailPage() {
  ...
  const handleAddNote = async () => {
    ...
    setAddingNote(true);
    try {
      const res = await fetch(`/api/sales-orders/${salesOrderId}/notes`, {
        method: 'POST',
  ...
  const handleAddSpec = async () => {
    ...
    setAddingSpec(true);
    try {
      const res = await fetch(`/api/sales-orders/${salesOrderId}/specs`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleAddScopeLine = async () => {
    if (!newScopeLine.text.trim()) return;
    setAddingScopeLine(true);
    try {
      const res = await fetch(`/api/sales-orders/${salesOrderId}/scope-lines`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  if (loading) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="text-center py-12 text-gray-500">Loading sales order...</div>
      </div>
    );
  }
    ...
    return (
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="bg-red-50 border border-red-200 rounded-lg p-6">
          ...
          <p className="text-red-700 text-sm mt-1">{error || 'Sales order not found'}</p>
          <Link href="/console/sales-orders" className="mt-3 inline-block text-red-700 hover:text-red-800">
  ...
  const currentSpecs = salesOrder.specs.filter((s: Spec) => s.is_current);
  const previousSpecs = salesOrder.specs.filter((s: Spec) => !s.is_current);
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      ...
      <div className="bg-purple-50 border border-purple-200 rounded-lg p-4 mb-6">
        <div className="flex items-center gap-3">
          ...
          <p className="text-purple-800">
            Created from Quote:{' '}
            {salesOrder.origin_quote ? (
              <Link href={`/console/quotes/${salesOrder.origin_quote.id}`} className="font-medium underline">
                {formatRef('quote', salesOrder.origin_quote.base_number)}
              </Link>
            ) : (
              <Link href={`/console/quotes/${salesOrder.origin_quote_id}`} className="font-medium underline">
                View Origin Quote
              </Link>
            )}
          </p>
      ...
      <div className="mb-6">
        <div className="flex items-center gap-4 mb-2">
          <Link href="/console/sales-orders" className="text-gray-500 hover:text-gray-700">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
            </svg>
          </Link>
          <h1 className="text-2xl font-bold text-gray-900">{formatRef('sales_order', salesOrder.base_number)}</h1>
        ...
        <div className="text-gray-600">
          {salesOrder.customer_name && <span className="mr-4">{salesOrder.customer_name}</span>}
          {salesOrder.customer_email && <span className="text-gray-400">{salesOrder.customer_email}</span>}
        </div>
      </div>
            ...
            >
              ...
              <span className="ml-1 text-gray-400">
                ({tab === 'specs' ? currentSpecs.length :
                  tab === 'notes' ? salesOrder.notes.length :
                  tab === 'attachments' ? salesOrder.attachments.length :
                  salesOrder.scope_lines.length})
              </span>
            </button>
          ))}
        </nav>
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        ...
        {activeTab === 'notes' && (
          <div>
            ...
            {/* Notes list */}
            {salesOrder.notes.length > 0 ? (
              <div className="space-y-4">
                {salesOrder.notes.map((note: Note) => (
          ...
          <div>
            ...
            {salesOrder.attachments.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {salesOrder.attachments.map((attachment: Attachment) => (
        ...
        {activeTab === 'scope' && (
          <div>
            ...
            {/* Scope lines list */}
            {salesOrder.scope_lines.length > 0 ? (
              <div className="space-y-2">
                {salesOrder.scope_lines.map((line: ScopeLine) => (
                  <div
                    key={line.id}
