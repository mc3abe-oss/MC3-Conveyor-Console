/**
 * Admin Caster Models Page
 *
 * Allows admin users to manage caster model catalog entries.
 * Features:
 * - Add new caster models (item_key, label, description, sort_order)
 * - Deactivate/reactivate models (toggle is_active)
 * - NO hard delete (data safety)
 *
 * Note: Same caster model can be used for both rigid and swivel configurations
 * in the application UI. This catalog just defines available caster models.
 */
...
const CATALOG_KEY = 'caster_model';

export default function AdminCasterModelsPage() {
  const [models, setModels] = useState<CasterModel[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [selectedModel, setSelectedModel] = useState<CasterModel | null>(null);
  ...
  async function loadModels() {
    ...
    try {
      ...
      if (!response.ok) throw new Error('Failed to fetch caster models');
      const data = await response.json();
      setModels(data);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to load caster models');
    } finally {
      setIsLoading(false);
    }
  }

  function selectModel(model: CasterModel) {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(err.error || 'Failed to save caster model');
      }
      ...
      if (isCreating) {
        ...
        const newModel = newModels.find((m: CasterModel) => m.item_key === formData.item_key);
        if (newModel) {
  ...
  if (isLoading) {
    return (
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <h1 className="text-2xl font-bold mb-4">Caster Models Admin</h1>
        <p>Loading...</p>
      </main>
    );
  }
  ...
  return (
    <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 className="text-2xl font-bold mb-6">Caster Models Admin</h1>
      <p className="text-gray-600 mb-6">
        Manage caster models available for conveyor support configurations.
        Same model can be used for both rigid and swivel caster selections in the application.
      </p>
            ...
            <div className="space-y-2 max-h-[60vh] overflow-y-auto">
              ...
              {models.length === 0 && (
                <p className="text-sm text-gray-500 text-center py-4">
                  No caster models found. Click &quot;+ New&quot; to add one.
                </p>
              )}
            </div>
          </div>
              ...
              <form onSubmit={handleSave} className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 border border-gray-300 rounded"
                      required
                      placeholder="e.g., Heavy Duty 5&quot; Caster"
                    />
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full px-3 py-2 border border-gray-300 rounded"
                    rows={2}
                    placeholder="e.g., 5&quot; polyurethane wheel, 400 lbs capacity per caster"
                  />
                </div>
                ...
                <div className="mt-4 p-3 bg-gray-50 border border-gray-200 rounded text-sm text-gray-600">
                  <strong>Note:</strong> Caster models cannot be deleted to preserve data integrity in
