...
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const result = await transitionScopeStatus('quote', id, status, userId);
    ...
    if (result.revision) {
      const latestRevision = await getLatestRevision('quote', id);
      if (latestRevision) {
        revisionInfo = {
 ...
 *
 * Get the current scope status and revision info for a quote.
 */
export async function GET(_request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const info = await getEntityStatusInfo('quote', id);
