...
const logger = createLogger().child({ module: 'api.delete-so-line' });
...
export async function DELETE(_request: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (isAlsoLinkedToQuote) {
      ...
      if (updateError) {
        logger.error('api.delete-so-line.deactivate.failed', { errorCode: ErrorCodes.DB_UPDATE_FAILED, error: updateError });
        return NextResponse.json(
      ...
      if (deleteError) {
        logger.error('api.delete-so-line.hard-delete.failed', { errorCode: ErrorCodes.DB_DELETE_FAILED, error: deleteError });
        return NextResponse.json(
    ...
    logger.error('api.delete-so-line.failed', { errorCode: ErrorCodes.API_INTERNAL_ERROR, error });
