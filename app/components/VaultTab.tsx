...
export default function VaultTab({ context, onOpenSaveModal }: VaultTabProps) {
  ...
  // Load vault data when context changes
  const loadVaultData = useCallback(async () => {
    ...
    const parentType = context.type;
    const parentId = context.id;

    try {
      const [notesRes, specsRes, scopeRes, attachRes] = await Promise.all([
        fetch(`/api/${parentType}s/${parentId}/notes`),
        fetch(`/api/${parentType}s/${parentId}/specs`),
        fetch(`/api/${parentType}s/${parentId}/scope-lines`),
        fetch(`/api/${parentType}s/${parentId}/attachments`),
      ]);
  ...
  const handleAddNote = async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch(`/api/${context.type}s/${context.id}/notes`, {
        method: 'POST',
  ...
  const handleAddSpec = async () => {
    if (!context || !newSpecKey.trim() || !newSpecValue.trim()) return;

    setSaving(true);
    try {
      const res = await fetch(`/api/${context.type}s/${context.id}/specs`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          key: newSpecKey.trim(),
  ...
  const handleAddScopeLine = async () => {
    if (!context || !newScopeText.trim()) return;

    setSaving(true);
    try {
      const res = await fetch(`/api/${context.type}s/${context.id}/scope-lines`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  // Add Attachment (metadata only for Phase 1)
  const handleAddAttachment = async () => {
    if (!context || !newAttachmentName.trim()) return;

    setSaving(true);
    try {
      const res = await fetch(`/api/${context.type}s/${context.id}/attachments`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          file_name: newAttachmentName.trim(),
          file_path: `/uploads/${context.type}s/${context.id}/${newAttachmentName.trim()}`,
    ...
    return (
      <div className="card">
        <div className="text-center py-12">
          ...
          <h3 className="mt-4 text-lg font-medium text-gray-900">Vault Unavailable</h3>
          <p className="mt-2 text-gray-500 max-w-md mx-auto">
            Link to a Quote or Sales Order to use the Vault. The Vault stores attachments, notes, specs, and scope lines.
          </p>
          <button
            onClick={onOpenSaveModal}
            className="mt-6 px-4 py-2 bg-blue-600 text-white font-medium rounded-lg hover:bg-blue-700 transition-colors"
          >
            Save to Quote/Sales Order
          </button>
        </div>
      </div>
    );
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Linked to {formatSaveTarget(context)}
          </p>
        </div>
      </div>
            ...
            >
              ...
              <span className="ml-1 text-xs text-gray-400">
                ({tab === 'notes' ? notes.length :
                  tab === 'specs' ? specs.filter(s => s.is_current).length :
                  tab === 'scope' ? scopeLines.length :
                  attachments.length})
              </span>
            </button>
          ))}
        ...
        <div className="card">
          ...
          {activeSubTab === 'specs' && (
            <div className="space-y-4">
              ...
              {/* Specs List */}
              {specs.filter(s => s.is_current).length === 0 ? (
                ...
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {specs.filter(s => s.is_current).map((spec) => (
                        <tr key={spec.id}>
          ...
          {activeSubTab === 'scope' && (
            <div className="space-y-4">
              ...
              {scopeLines.length === 0 ? (
                ...
                <div className="space-y-2">
                  {scopeLines.sort((a, b) => a.position - b.position).map((line) => (
